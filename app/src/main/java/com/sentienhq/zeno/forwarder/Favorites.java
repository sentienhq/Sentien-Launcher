...
import com.sentienhq.zeno.result.AppResult;
...
public class Favorites extends Forwarder implements View.OnClickListener, View.OnLongClickListener, View.OnTouchListener, View.OnDragListener {
    ...
    void onFavoriteChange() {
        favoritesPojo = ZenoApplication.getApplication(mainActivity).getDataHandler()
                .getFavorites();
    ...
    private void addDefaultAppsToFavs() {
        {
            ...
            if (resolveInfo != null) {
                ...
                if (resolveInfo.activityInfo.name != null && !resolveInfo.activityInfo.name.equals(DEFAULT_RESOLVER)) {
                    ...
                    ZenoApplication.getApplication(mainActivity).getDataHandler().addToFavorites("app://" + packageName + "/" + activityName);
                }
            }
        }
        {
            ...
            if (resolveInfo != null) {
                ...
                if (resolveInfo.activityInfo.name != null && !resolveInfo.activityInfo.name.equals(DEFAULT_RESOLVER)) {
                    ZenoApplication.getApplication(mainActivity).getDataHandler().addToFavorites("app://" + packageName + "/" + resolveInfo.activityInfo.name);
                }
            }
    ...
    public boolean onDrag(View v, final DragEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case DragEvent.ACTION_DRAG_ENDED:
                ...
                final int pos = ZenoApplication.getApplication(mainActivity).getDataHandler().getFavoritePosition(overApp.result.getPojoId());

                draggedView.post(() -> {
                    // Signals to a View that the drag and drop operation has concluded.
                    // If event result is set, this means the dragged view was dropped in target
                    if (event.getResult()) {
                        ZenoApplication.getApplication(mainActivity).getDataHandler().setFavoritePosition(mainActivity, draggedApp.result.getPojoId(), leftSide ? pos - 1 : pos);
                        mainActivity.onFavoriteChange();
                    } else {
